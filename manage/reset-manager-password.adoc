...
+
Managerパスワードをリセットするには、ONTAP tools for VMware vSphereメンテナンスコンソールを使用してリセットトークンを生成する必要があります。..vCenter Serverで、メンテナンスコンソールを開きます。「2」と入力して[System Configuration]オプションを選択します。「3」を入力してトークンを生成し、マネージャのパスワードをリセットします。
